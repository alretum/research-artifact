...
public class ModelRegistry {
    ...
    /**
     * A model resolved to the client that reaches it.
     *
    ...
    private static void requireKey(Backend backend) {
        ...
        if (key == null || key.isBlank()) {
            throw new IllegalStateException("Backend '" + backend.name() + "' needs $" + backend.apiKeyEnv()
                    + ", which is unset. Export it before running; it is never read from a file in the repository.");
        }
    }
}
